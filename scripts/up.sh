...
# Detect whether ECR already has images so we can show the right next steps.
# config-server is the first service to start — if it has an image, all services do.
ECR_IMAGE_TAG=$(aws ecr list-images \
  --repository-name "petclinic-${ENV}/config-server" \
  --region eu-central-1 \
  --query 'imageIds[?imageTag!=`null`].imageTag | [0]' \
  --output text 2>/dev/null || echo "")

echo ""
echo "============================================================"
echo "  petclinic-$ENV infrastructure is UP"
echo "============================================================"
echo ""
echo "  ┌─ Access points ─────────────────────────────────────────────────┐"
echo "  │                                                                 │"
printf "  │  App URL:   %-52s│\n" "$APP_URL"
echo "  │                                                                 │"
echo "  │  ArgoCD:    kubectl port-forward svc/argocd-server -n argocd \\│"
echo "  │               8443:443                                          │"
printf "  │             https://localhost:8443  (admin / %-18s)│\n" "$ARGOCD_PASS"
echo "  │                                                                 │"
echo "  │  Grafana:   kubectl port-forward svc/grafana -n monitoring \\  │"
echo "  │               3000:3000                                         │"
echo "  │             http://localhost:3000  (admin / petclinic-admin)    │"
echo "  │                                                                 │"
echo "  │  Zipkin:    kubectl port-forward svc/zipkin -n tracing \\      │"
echo "  │               9411:9411   →   http://localhost:9411             │"
echo "  └─────────────────────────────────────────────────────────────────┘"
echo ""

if [[ -z "$ECR_IMAGE_TAG" || "$ECR_IMAGE_TAG" == "None" ]]; then
  # ── ECR is empty → pods will be in ImagePullBackOff ──────────────────────
  echo "  ╔═════════════════════════════════════════════════════════════════════╗"
  echo "  ║  ACTION REQUIRED — $APP_URL shows 503  ║"
  echo "  ║  until Docker images are built and pushed to ECR.                  ║"
  echo "  ╚═════════════════════════════════════════════════════════════════════╝"
  echo ""
  echo "  WHY: ECR repos were just created empty by Terraform. ArgoCD already"
  echo "  auto-synced and scheduled pods, but they are in ImagePullBackOff"
  echo "  because there are no images to pull yet. This is expected on every"
  echo "  fresh up.sh run after a destroy."
  echo ""
  echo "  NEXT STEPS (do these in order):"
  echo ""
  echo "  ① Build and push images to ECR — trigger CI in your app repo fork:"
  echo "      GitHub → your spring-petclinic-microservices fork"
  echo "              → Actions → 'CI - Build and Push' → Run workflow"
  echo "                Set:  force_rebuild_all = true"
  echo "                Then: Run workflow"
  echo "      Wait ~10 min for ARM64 builds to finish."
  echo ""
  echo "  ② Verify update-image-tags.yml auto-triggered (pull first):"
  echo "      git pull && git log --oneline -3"
  echo "      Look for: 'ci: update image tags to <sha> (...)'"
  echo ""
  echo "  ③ If that commit is missing (dispatch didn't fire), trigger manually:"
  echo "      GitHub → this platform repo"
  echo "              → Actions → 'Update Image Tags' → Run workflow"
  echo "                sha: <7-char SHA from ECR or CI logs>"
  echo "                services: (leave as default — all 8)"
  echo ""
  echo "  ④ Watch pods come up (auto-sync fires within ~3 min of tag commit):"
  echo "      kubectl get pods -n $NAMESPACE --watch"
  echo "      All 8 should reach 1/1 Running within ~5 min."
  echo ""
  echo "  ⑤ Run the smoke test to verify everything end-to-end:"
  echo "      bash scripts/smoke-test.sh --env $ENV"
  echo ""
  echo "  ⑥ Open the app:"
  echo "      $APP_URL"
  echo ""
else
  # ── ECR has images → ArgoCD will deploy shortly ──────────────────────────
  echo "  ECR already has images (latest tag: ${ECR_IMAGE_TAG})."
  echo "  ArgoCD is syncing — pods will be Running within ~5 min."
  echo ""
  echo "  Watch:  kubectl get pods -n $NAMESPACE --watch"
  echo "  Smoke:  bash scripts/smoke-test.sh --env $ENV"
  echo "  App:    $APP_URL"
  echo ""
fi

echo "  ─────────────────────────────────────────────────────────────────────"
echo "  NOTE: Alertmanager is running with a null receiver (no email alerts)."
echo "  To enable email notifications after the app is live:"
echo "    aws secretsmanager create-secret \\"
echo "      --name petclinic/alertmanager-smtp \\"
echo "      --secret-string '{\"email\":\"you@gmail.com\",\"password\":\"app-password\"}' \\"
echo "      --region eu-central-1"
echo "    bash scripts/install-observability.sh --env $ENV"
echo "  ─────────────────────────────────────────────────────────────────────"
echo ""
echo "============================================================"
