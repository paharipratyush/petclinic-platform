...
ROLE_ARN=$(terraform -chdir="$TF_DIR" output -raw lb_controller_role_arn)
...
for i in $(seq 1 24); do
  ...
  if [[ -n "$ALB_ADDRESS" ]]; then
    ...
    echo ""
    echo "==> NEXT STEP — Create the Route 53 A record:"
    echo "    Set var.alb_dns_name in terraform/environments/$ENV/main.tf (or pass as -var), then:"
    echo ""
    ...
    if [[ "$ENV" == "prod" ]]; then
      DOMAIN=$(terraform -chdir="$TF_DIR" output -raw app_url | sed 's|https://||')
    else
      DOMAIN=$(terraform -chdir="$TF_DIR" output -raw app_url | sed 's|https://||')
    fi
