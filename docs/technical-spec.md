...
> **Implementation note:** The original design used Route 53 as authoritative DNS. Because the domain is registered via Cloudflare Registrar, which does not support NS delegation to external providers, we use the Cloudflare Terraform provider instead. See [ADR-0013](adr/0013-cloudflare-provider-for-dns.md).
...
| ADR-0013 | Cloudflare Terraform provider for DNS | Accepted | Route 53 replaced by Cloudflare provider because the domain is on Cloudflare Registrar (no NS delegation support). Fully automated cert validation. |
